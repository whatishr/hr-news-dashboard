...
def summarize_with_gpt(title, raw_description):
    """[핵심] GPT가 뉴스 제목과 내용을 해석하여 인사담당자용 1~2줄 요약을 직접 작성하도록 강화된 프롬프트"""
    if not client or not OPENAI_API_KEY:
        print("⚠️ OpenAI API Key가 없어 원본 description을 사용합니다.")
        return raw_description

    try:
        # GPT에게 정확하게 역할을 부여하고 결과 양식을 지정
        prompt = f"""
...
2. '~라는 소식입니다' 같은 입사체보다는 '~함에 따라 대비가 필요함', '~가 확대되는 추세임' 등 인사 실무 관점의 명확한 서술어로 작성해 주세요.
3. 오직 요약문 결과만 출력하세요.
"""
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 게임/플랫폼 업계 전문 HR 아날리스트입니다. 뉴스의 핵심 시사점을 짧고 명확하게 요약합니다.",
                },
        ...
        print(f"⚠️ GPT 요약 생성 실패 ({title[:15]}...): {e}")
        return raw_description
...
def fetch_top_hr_news(limit_total=10):
    ...
    for category, query in HR_SEARCH_TARGETS.items():
        ...
        url = f"https://openapi.naver.com/v1/search/news.json?query={encoded_query}&display=20&sort=sim"
        headers = {
    ...
    print("🤖 GPT 기반 HR 핵심 요약문 생성 중...")
    final_articles = []
    for art in unique_articles:
        # GPT를 통한 정밀 1~2줄 요약문 생성
        ...
        final_articles.append(
            {
                ...
                "title": art["title"],
                "description": gpt_summary,  # 고품질 GPT 요약문 저장
                "link": art["link"],
                "pubDate": art["pubDate"],
    ...
    if not df.empty:
        ...
        print(
            f"🎉 [성공] GPT 정밀 요약이 완료된 뉴스 {len(df)}건을 저장했습니다!"
        )
    else:
